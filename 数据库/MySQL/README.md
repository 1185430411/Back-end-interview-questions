...
##### 3.事务的并发？事务隔离级别？每个级别会引发什么问题？MySQL默认是哪个级别？
		从理论上来说，事务之间应该彼此隔离，以避免因为事务并发而导致的问题。但是这样的话会对性能产生很大的
	影响。在实际开发中，为了提升性能，事务通常还会以较低的隔离级别来运行。
		事务隔离级别：
			1.读未提交：顾名思义，事务可以读到其他事务未提交的修改，这是最低等级的隔离级别。会导致脏读：事务A
			读到了事务B未提交的修改，然后事务B回滚了，那么事务A读到的数据就是脏数据。此时写数
			2.读已提交：只有等一个事务提交了，另一个事务才能进行读操作。解决了脏读，但会导致不可重复读：事务A
			正在多次读取同一个数据，此时事务B对同一个数据进行了更新操作并提交。那么事务A前后读到的数据内容不
			相等，且之前读到的数据不可被复现。
			3.可重复读：只有一个事务的读操作完成了，另一个事务才能进行修改操作。解决了不可重复读的问题，但会
			导致幻读：事务A多次进行读操作时，事务B进行了插入或者删除操作，使得事务A前后读到的数据不相等，
			仿佛产生了幻觉，这就是幻读。幻读和不可重复读的区别就是，不可重复读并发事务的修改造成的，而
			幻读是并发事务的插入或者删除造成的。
			4.串行化：最高的隔离级别。事务之间按照顺序一个个执行，解决了三种并发问题，但会降低性能。
			
		MySQL默认的事务隔离级别为可重复读，另外：
